...
This repository also contains the retired Rails surface from an earlier Flyd product. The TypeScript coding/runtime substrate under `cli/src/runtime/` is still active and shared by the Mac product and CLI dogfood surface; Rails under `app/`, `bin/rails`, `db/`, `lib/`, and `test/` is legacy only.

If you're an agent navigating this repo cold: start in `mac-adapter/`, `cli/src/server.ts`, and `cli/src/cognition/`. The active brain is the TypeScript Cognitive Core; Mac, LIVE, CLI conversation, task planning, coding memory, and Work Intelligence consume the same compiled context path.
...
The Swift adapter never decides what to do — all intelligence routes through TypeScript Core. The Cognitive Core is:

`immutable events → claims + typed relations → derived world state → PROFILE/NOW/project projections + materialized Present → Context Compiler → model/tools`.

Jev is an optional bounded System-1 predicate evaluator for semantic yes/no/choice judgments. It never owns lifecycle, permissions, dates, graph traversal, or destructive-action safety; deterministic code remains authoritative. Hosted Jev egress is opt-in via `FLYD_JEV_ENABLED=true`.

Core loads a shared secret from `~/.flyd/overlay/auth-token` (generated by the adapter on first run) and requires it as a `Bearer` token on every request.
...
| `FLYD_JEV_ENABLED` | `false` | Opt in to hosted Jev System-1 predicate calls |
| `FLYD_JEV_MODEL` | `jev-latest` | Jev model override when enabled |
| `FLYD_JEV_ENDPOINT` | Typesafe System-1 endpoint | Jev endpoint override when enabled |

...
model's live response while it is being generated. That response may naturally
open with a context-sensitive phrase such as “Let me think about this” or
“Okay, I’m looking into it” when it fits the turn. It must not play a fixed
...
- Every conversational turn uses one configured Realtime voice. The default is
  `marin`.
- A conversational lead-in is allowed only when the Realtime model generates it
  as part of the same intelligent response and continues naturally into the
  answer.
- Flyd never injects a hard-coded phrase, starts a separate acknowledgement
  request, or forces the same lead-in on every turn.
...
- direct, conversational response instructions that permit brief, varied
  lead-ins when natural but never require one;
...
The model is instructed to behave like a present conversational partner rather
than a status system. It may acknowledge the shape of a difficult or
tool-dependent request while it begins working, but the acknowledgement and
answer are one continuous generated response. It should answer simple questions
directly and avoid repetitive verbal tics.
...
There is no separate synthetic spoken processing state. The panel supplies
immediate visual acknowledgement, while any spoken lead-in comes naturally
from the same streamed Realtime response as the answer.
...
- the first audible content is generated by the active Realtime response and is
  either the direct answer or a natural, context-sensitive lead-in;
- audio starts before the complete answer exists;
- the voice remains the same for the whole response and subsequent turns;
- conversational lead-ins vary by context and are not injected into every turn;
...
- session instructions allow natural same-response lead-ins without mandating a
  stock phrase;
...
- No canned, separately generated, or mandatory acknowledgement is added before
  the answer. Natural language generated inside the one live response is in
  scope.

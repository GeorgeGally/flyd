...
class Surface::PlannerTest < ActiveSupport::TestCase
  test "creates a focused semantic surface from remembered state" do
    ...
    project.decisions.create!(conversation:, source_message: message, content: "Replace project-first navigation", confidence: 0.9)

    surface = Surface::Planner.call

    assert_equal "project-#{project.id}", surface.focus_item_id

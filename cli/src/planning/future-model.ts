...
/** Consequence-prediction boundary. It never grants execution authority. */
...
export function assessConfidence(input: {
  ...
  let points = 0;
  if (input.sourceFresh) { points += 2; reasons.push("source data is fresh"); }
  else reasons.push("source freshness is uncertain");
  ...
  return {
    level: points >= 4 ? "high" : points >= 1 ? "medium" : points >= -1 ? "low" : "unknown",
    reasons,
  };
}

/**
 * Baseline consequence model. It only predicts explicitly declared state
 * transitions. A semantic/learned predictor can implement FutureModel later
 * without changing callers or execution authority.
 */
  ...
  async predict({
    ...
    for (const effect of candidateAction.deterministicEffects ?? []) {
      ...
      effects.push({
        ...
        before,
        after: effect.value,
        rationale: "declared deterministic action effect",
      });
    }
    const deterministic = effects.length > 0;
    return {
      id: randomUUID(),
      snapshotId: currentState.id,
      ...
      risks: deterministic ? [] : ["Action effects are not modeled yet"],
...
export function reconcilePrediction(
  ...
  return {
    ...
    correctEffects: correct,
    incorrectEffects: incorrect,
    missedEffects: missed,
...
export function buildPlanningTrace(input: {
  ...
  return {
    ...
    goal: input.goal,
    candidates: ranked,
    chosenActionId: ranked[0]?.action.id,
    rejectedActionIds: ranked.slice(1).map((candidate) => candidate.action.id),
...
export class MultiStepPlanner {
  constructor(
    private readonly futureModel: FutureModel,
    private readonly evaluator = new ActionEvaluator(),
  ) {}
  ...
  async plan(input: {
    ...
    let frontier: CandidatePlan[] = [{
      ...
      accumulatedRisk: 0,
      userEffort: 0,
      confidence: { level: "high", reasons: ["initial observed state"] },
      assumptions: [],
    }];
    ...
    for (let depth = 0; depth < maxDepth; depth += 1) {
      ...
      for (const plan of frontier) {
        for (const action of input.actions) {
          ...
          const prediction = await this.futureModel.predict({
            currentState: plan.finalState,
          ...
          const evaluation = this.evaluator.evaluate(
            action,
            prediction,
            input.score(action, prediction, depth),
          );
          const steps = [...plan.steps, { action, prediction, evaluation }];
          next.push({
            id: randomUUID(),
            steps,
            finalState: prediction.predictedState,
...
export type { PlanningConfidence, WorldStateSnapshot } from "../intelligence/world/types.js";

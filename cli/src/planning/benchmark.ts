...
export const PLANNING_BENCHMARK: PlanningBenchmarkScenario[] = [
  ...
  { name: "empirical verification reliability", goal: "choose the path that usually verifies", expectedActionId: "reliable", actions: [{ action: action("flaky", "repeat historically flaky execution"), scores: score({ progress: .9, reachability: .3, risk: .8, confidence: .8 }) }, { action: action("reliable", "use historically reliable execution"), scores: score({ progress: .75, reachability: .92, risk: .12, confidence: .85 }) }] },
  ...
  { name: "unresolved contextual request", goal: "continue coding work", expectedMode: "ask_user", actions: [{ action: action("guess", "guess what 'continue' refers to"), scores: score({ progress: .8, confidence: .1 }) }], gaps: [{ id: "coding-outcome", kind: "user_preference", description: "no active task resolves the contextual request", severity: "high", blocking: true, evidenceNeeded: "concrete intended outcome" }] },
];

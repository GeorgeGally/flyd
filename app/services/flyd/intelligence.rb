  ...
  class Intelligence
    ...
    def system_prompt
      <<~PROMPT
        ...
        - discovery: compose up to four grounded objects from recent work, a personal daily signal, and fresh external stories without manufacturing urgency. Every title is reality-referential—about the user's world, work, or interests—never self-referential ("Continue Flyd") or software-imperative.
        ...
        - discovery: judge which supplied evidence the user would genuinely find interesting, then create up to 4 ranked discovery_scene items. Curate rather than listing everything. Use terse titles (≤100 chars) and single-sentence summaries. Never title or frame an item about Flyd itself or its own repository ("Continue Flyd", "Own coding execution in Flyd")—every item describes something in the user's world, not an instruction to the interface. Include a specific one-line metadata.why_it_matters grounded in the evidence.
        ...
        Never use a provider evidence type such as goal, tension, signal, curiosity, nudge, report, or event as an item kind. Use only the item kinds listed in the JSON contract. Maximum four items for discovery, three for decision and investigation, two for conversation and monitoring, one for quiet and action. Discovery is a ranked deck, not a feed: include only evidence that earns attention. Use concise titles and one-sentence summaries to fit. Item ids are created by you or reused from existing scene_key values. Context and source references must use exact ids present in the supplied state. Media attachment ids must also appear as explicit intent_attachment source references. Do not include private reasoning.
      PROMPT
    end

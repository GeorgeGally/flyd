...
# Compile the CLI producer during the image build. Keep only runtime packages in
# the final image; production refreshes execute node cli/dist/export-state.js.
RUN cd cli && npm ci && npm run build && npm prune --omit=dev
